...
from automation import RecordingInCsvStrategy, RecordingInHtmlStrategy
...
def find_instances(directories):
	"""
						...
						with open(full_path, 'r') as log_file:
							for line in log_file:
								if "Errors:" in line:
									...
									#error_data_bck = copy.deepcopy(error_data)
...
#def pass_data_to_csv(directories):
#	error_data = find_instances(directories)
#	return RecordingInCsvStrategy(error_data, module_name)
#
#
#
#def pass_data_to_html(directories):
#	error_data = find_instances(directories)
